...
function CollectionIActivityPane(props: any) {
  ...
  const [options, setOptions] = useState({
    ...
    xaxis: { categories: ["02/28", "03/05", "03/10", "03/15", "03/20", "03/25", "04/01"] },
  });
  const [series, setSeries] = useState([
    { name: "series-1", data: [1.8, 1.7, 0.8, 1.4, 1.5, 1.7, 1.0] },
  ]);
  return (
              ...
              <thead>
                <tr>
                  {heads.map((item, index) => (
                    <th
                      className={`text-center ${theme === "dark" ? "text-light" : "text-dark"}`}
                      key={index}
                    >
                      {item}
                    </th>
